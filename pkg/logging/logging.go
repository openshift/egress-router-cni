...
func Panicf(format string, a ...interface{}) {
	...
	Printf(PanicLevel, "%+v", errors.New("Egress Router CNI Panic"))
...
func getLoggingLevel(levelStr string) Level {
	...
	fmt.Fprintf(os.Stderr, "Egress Router CNI logging: cannot set logging level to %s\n", levelStr)
	return UnknownLevel
}
...
func SetLogFile(filename string) {
	...
	if err != nil {
		loggingFp = nil
		fmt.Fprintf(os.Stderr, "Egress Router CNI logging: cannot open %s", filename)
	}
	loggingFp = fp
}
